...
For example, if I want to read [beewing.txt](`r here::here("data", "beewing.txt")`) (which are left wing widths of 100  honey bees in mm) in:
...
* Save a copy of [beewing.txt](`r here::here("data", "beewing.txt")`), read it in and check the structure
```{r}
file <- here::here("data", "beewing.txt")
bee <- read.table(file, header = TRUE)
str(bee)
...
# Exercise
The data in [chaff.txt](`r here::here("data", "chaff.txt")`) are the masses of male and female chaffinches. It is organised in to two columns, `males` and `females` a format which is not normally ideal. Your task is to organise the analysis of these data.  
You need to: 
...
[Rmd file](`r here::here("workshops", "01_project_organisation.Rmd")`)
